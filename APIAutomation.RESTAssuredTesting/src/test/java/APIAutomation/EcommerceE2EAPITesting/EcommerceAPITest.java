...
//---Note: here paylod in form of form parameter so use pram() on place of body()		
		...
		.param("productName", "HeadPhones")
		.param("productAddedBy", userId)
		.param("productCategory", "fashion")
		.param("productSubCategory", "Electronic")
		.param("productPrice", "1650")
		.param("productDescription", "Boat")
...
//----- Serialization	by Setter Getter method POJO class ------------		
		...
		String orderespnse = createOrderequest.when().post("/api/ecom/order/create-order").
		then().log().all().extract().response().asString();
...
//--- DELETE() ------- Delete Product ---------------------
		...
		RequestSpecification productDeleteeq = given().log().all().spec(deleteProductReq).pathParam("poductId", productId);
		
		//Took productId as key from PathParm() method - and used in enp point url -----------------
		String deleteProductes = productDeleteeq.when().delete("/api/ecom/product/delete-product/{poductId}").
		then().log().all().extract().response().asString();
	
		System.out.print("deleteProductes :"+deleteProductes);
	}
